...
function OpenDrugShop()
	local elements = {
		{unselectable = true, icon = "fas fa-meth", title = TranslateCap('dealer_title')}
	}
	menuOpen = true
	...
	ESX.OpenContext("right", elements, function(menu,element)
		...
		ESX.OpenContext("right", elements2, function(menu2,element2)
			ESX.CloseContext()
			TriggerServerEvent('hw_meth:sellDrug',tostring(element.name),tonumber(menu2.eles[2].inputValue))
		end, function(menu)
			menuOpen = false
		end)
	end, function(menu)
...
function OpenBuyLicenseMenu(licenseName)
	...
	ESX.OpenContext("right", elements, function(menu,element)
		ESX.TriggerServerCallback('hw_meth:buyLicense', function(boughtLicense)
